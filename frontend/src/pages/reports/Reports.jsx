  ...
  return (
    <div className="space-y-6">
            ...
            {['DAILY','WEEKLY','MONTHLY','YEARLY'].map(p => (
              <button key={p} onClick={() => setPeriod(p)}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-medium transition-all',
                  period===p ? 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm'
      ...
      {summaryLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <SummaryCard label="Total Income"   value={formatCurrency(summary.totalIncome, currency)} color="text-emerald-600 dark:text-emerald-400" sub={`${summary.transactionCount} transactions`} />
          <SummaryCard label="Total Expenses" value={formatCurrency(summary.totalExpense, currency)} color="text-red-500" />
          <SummaryCard label="Net Savings"    value={formatCurrency(summary.netSavings, currency)}
      ...
      <div className="card p-5">
        ...
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <Input label="Start Date" type="date" value={dateRange.startDate}
            onChange={e => setDateRange(p => ({...p, startDate: e.target.value}))}/>
          <Input label="End Date" type="date" value={dateRange.endDate}
